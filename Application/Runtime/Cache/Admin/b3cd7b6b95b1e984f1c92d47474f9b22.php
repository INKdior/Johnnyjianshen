...
<div id="frameBord" name="frameBord">
...
<div class="main-content">
    ...
    <div class="add_cp">
        ...
        <form style="float:left;margin:10px;" method="get" action="<?php echo U('admin/index/deliverylist',array('order_id'=>$order_id));?>">
    ...
    <div class="table_con">
       <table>
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form action="<?php echo U('admin/index/delivery_details');?>" delivery_id="<?php echo ($vo[delivery]["id"]); ?>" method="get">
                    <tr>
                        <input type="hidden" id="delivery_id" name="id" value="<?php echo ($vo[delivery]["id"]); ?>" >
                        <td><?php echo ($vo[delivery]["name"]); ?></td>
                        <td><?php echo ($vo[delivery]["phone"]); ?></td>
                        <td id="state"><?=$vo[delivery][data_state] == '0' ? '启用' : '禁用'?></td>
                        <td>
                            <a href="javascript:;" class="state" state="undone" id="<?php echo ($vo[delivery]["id"]); ?>">未完成（<?php echo ($vo['undone']); ?>）</a>
                            <a href="javascript:;" class="state" state="done" id="<?php echo ($vo[delivery]["id"]); ?>">已完成（<?php echo ($vo['done']); ?>）</a>
...
<script type="text/javascript">
    ...
    $('.call').on('click', function() {
        ...
        $.post('<?php echo U("admin/index/call_delivery");?>',{order_id:order_id,delivery_id:delivery_id},function() {
            window.opener.location.reload();
            window.close();
        });
    })
    $('#add').click(function() {
        window.location.href = "<?php echo U('admin/index/delivery_details');?>";
    });
    $('.state').click(function() {
        var state = $(this).attr('state');
        var delivery_id = $(this).attr('id');
        if(order_id != '') {
            window.open("<?php echo U('admin/index/order');?>?state="+state+"&delivery_id="+delivery_id,'','width=800,height=400');
        } else {
            window.location.href = "<?php echo U('admin/index/order');?>?state="+state+"&delivery_id="+delivery_id;
        }
    })
    $('.upd').click(function() {
    ...
    $('.ban').click(function() {
        ...
        var ban = $(this);
        if(confirm('是否禁用该配送员！')) {
            $.post('<?php echo U("admin/index/ban");?>',{id:delivery_id},function(json) {
    ...
    $('.reuse').click(function() {
        ...
        var reuse = $(this);
        if(confirm('是否启用该配送员！')) {
            $.post('<?php echo U("admin/index/reuse");?>',{id:delivery_id},function(json) {
