...
<div class="main-content">
    ...
    <div class="add_cp">
         <input type="button"  class="btn btn-primary xs" value="添加管理员">
    </div>
    <div class="table_con">
       <table>
            <tr class="tb_title">
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form action="<?php echo U('admin/index/admin_details');?>" method="post">
                        <input type="hidden" id="aid" name="id" value="<?php echo ($vo['id']); ?>" >
                    <tr>
                        <td><?php echo ($vo["name"]); ?></td>
                        <td style="width:20%">
                            <?php
 if($vo['level'] == '0') { echo '配送员'; } else if($vo['level'] == '1') { echo '管理员'; } ?>
                        </td>
                        <td align="center" valign="middle"> 
                            <input type="submit" value="修改"  style="width:50px; height:38px; line-height:38px; margin-top:1px; margin-bottom:1px; background-color:#C4DEF4; color:#2a6496">
                            <a style="float:left" class="del" id="<?php echo ($vo['id']); ?>" href="javascript:;" >删除</a>
                        </td>
                    </tr>
...
<script type="text/javascript">
    $('.add_cp').click(function() {
        window.location.href = "<?php echo U('admin/index/addadmin');?>";
    })
    $('.del').click(function() {
        var id = $(this).attr('id'); 
        if(confirm('是否删除该管理员！')) {
            window.location.href = "<?php echo U('admin/index/del');?>?id="+id+"&key=admin";
        }
    })
</script>
